...
<img src="/posts/res/day11/eniac.jpg" style="zoom:50%;">
...
Python 中还允许在`\`后面还可以跟一个八进制或者十六进制数来表示字符，例如`\141`和`\x61`都代表小写字母`a`，前者是八进制的表示法，后者是十六进制的表示法。另外一种表示字符的方式是在`\u`后面跟 Unicode 字符编码，例如`\u9a86\u660a`代表的是中文“tp”。运行下面的代码，看看输出了什么。
...
print(b.decode('utf-8'))  # tp
print(c.decode('gbk'))    # tp
```
